        ...
        for(csp = s->cstats; csp != NULL; csp = csp->next) {
           ...
           for(fp = csp->freqs; fp != NULL; fp = fp->next) {
              ...
              if(csp->tallied) {
                ...
                fprintf(fd, "(%6.2f,***.**)", fp->score);   // removed (float)fp->numless * 100.0/csp->tallied
              }
           }
           fprintf(fd, "\n");
           if(csp->sstats != NULL && csp->sstats->next == NULL)
...
int nm;
{
        ...
        for(stp = c->roster; stp != NULL; stp = stp->cnext) {
           fprintf(fd, "%-16.16s%c  %-16.16s (%-12.12s, Section %-8.8s)\n",
                       (nm>0) ? "" : stp->surname, (nm>0) ? ' ' : ',',
                       (nm>0) ? "" : stp->name, stp->id, stp->section->name);
...
int nm;
{
        ...
        for(stp = c->roster; stp != NULL; stp = stp->cnext) {
          fprintf(fd, "%6.2f %-16.16s%c %-16.16s (%-12.12s, Section %-8.8s)\n",
                   stp->composite, (nm>0) ? "" : stp->surname, (nm>0)? ' ' : ',',
                   (nm>0) ? "" : stp->name, stp->id, stp->section->name);
        }
        fprintf(fd, "\n");
}
...
float min, max;
{
    ...
    for(col = 0; col < 50; col++) {
        // printf("value of bins[%d] is %d \n", col, bins[col]);
        // printf("value of cmax is %d \n", cmax);
...
void reporttabs(FILE *fd, Course *c)    // removed nm
{
        Assignment *ap;
        Student *stp;
        Score *rscp; //, *nscp;
